...
@section('page-subtitle', 'Salary disbursement summary')
...
@section('content')
        ...
        <div class="card-body">
            <form method="GET" class="row g-2 align-items-end">
                ...
                <div class="col-md-3 d-flex gap-1">
                    ...
                    <a href="{{ route('reports.payroll', array_merge(request()->query(), ['export' => 1])) }}" class="btn btn-sm btn-outline-success"><i
    ...
    <div class="stat-grid">
        <div class="stat-card stat-blue">
            <div class="stat-inner">
                ...
                <div>
                    ...
                    <div class="stat-value">₹{{ number_format($totals->gross ?? 0, 0) }}</div>
                    <div class="stat-sub">{{ \Carbon\Carbon::create()->month($month)->format('F') }} {{ $year }}
                    </div>
                </div>
            </div>
        </div>
        <div class="stat-card stat-red">
            <div class="stat-inner">
                ...
                <div>
                    ...
                    <div class="stat-value">₹{{ number_format($totals->deductions ?? 0, 0) }}</div>
                    <div class="stat-sub">{{ \Carbon\Carbon::create()->month($month)->format('F') }} {{ $year }}
                    </div>
                </div>
            </div>
        ...
        <div class="stat-card stat-green">
            <div class="stat-inner">
                ...
                <div>
                    ...
                    <div class="stat-value">₹{{ number_format($totals->net ?? 0, 0) }}</div>
                    <div class="stat-sub">{{ \Carbon\Carbon::create()->month($month)->format('F') }} {{ $year }}
                    </div>
                </div>
            </div>
        ...
        <div class="stat-card stat-purple">
            <div class="stat-inner">
                <div class="stat-icon-wrap"><i class="bi bi-receipt"></i></div>
                <div>
                    <div class="stat-label">Records</div>
                    <div class="stat-value">{{ $totals->count ?? 0 }}</div>
                    <div class="stat-sub">{{ \Carbon\Carbon::create()->month($month)->format('F') }} {{ $year }}
                    </div>
                </div>
            </div>
                    ...
                    <tbody>
                        @forelse($payrollRecords as $rec)
                            @php
                                ...
                                $monthName = \Carbon\Carbon::create($rec->year, $rec->month)->format('M Y');
                            @endphp
                            <tr>
                                <td><strong>{{ $rec->employee->employee_code }}</strong><br><small>{{ $rec->employee->full_name ?? '—' }}</small>
                                </td>
                                <td><small>{{ $rec->employee->department->name ?? '—' }}</small></td>
                                <td>{{ $monthName }}</td>
                                <td class="text-end">{{ number_format($rec->gross_earnings, 2) }}</td>
                                ...
                                <td><span
                                        class="badge bg-{{ $sc }}-subtle text-{{ $sc }}">{{ ucfirst($rec->status) }}</span>
                                </td>
                                <td><a href="{{ route('payroll.payslip', $rec) }}" class="btn btn-sm btn-outline-info"><i
                                            class="bi bi-file-earmark-text"></i></a></td>
                            </tr>
                        @empty
                            <tr>
                                <td colspan="8" class="text-center text-muted py-4">No payroll records for the selected
                                    filters.</td>
                            </tr>
                        @endforelse
                    </tbody>
    ...
    <div class="d-flex justify-content-end mt-2">{{ $payrollRecords->withQueryString()->links() }}</div>
@endsection
